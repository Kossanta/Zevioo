 ...
 */
public class CheckVFCode extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        mInformer.onStart(WSTool.CHECK_VFCODE);
    }

    @Override
    protected void onPostExecute(Void s) {
        super.onPostExecute(s);
        mInformer.onEnd(WSTool.CHECK_VFCODE, 0);
    }
    ...
    private void sendData() {
        try {
            // 1. URL
            URL url = new URL(WSTool.SERVER_URL + "/checkvfcode");
            // 2. Open connection
            ...
            if (responseJSON.getInt("RES") == 0 && responseCode == 200) {
                mInformer.onSuccess(WSTool.CHECK_VFCODE, responseJSON);
            } else {
                mInformer.onError(WSTool.CHECK_VFCODE, responseJSON);
            }
