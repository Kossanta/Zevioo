 ...
 */
public class GetSuggestions extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        mInformer.onStart(WSTool.GET_SUGGESTIONS);
    }

    @Override
    protected void onPostExecute(Void s) {
        super.onPostExecute(s);
        mInformer.onEnd(WSTool.GET_SUGGESTIONS);
    }
    ...
    private void sendData() {
        try {
            // 1. URL
            URL url = new URL(WSTool.SERVER_URL + "/suggest");
            // 2. Open connection
            ...
            if (responseJSON.getInt("RES") == 0 && responseCode == 200) {
                mInformer.onSuccess(WSTool.GET_SUGGESTIONS, responseJSON);
            } else {
                mInformer.onError(WSTool.GET_SUGGESTIONS, responseJSON);
            }
