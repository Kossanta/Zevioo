...
public class GetLanguages extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        mInformer.onStart(WSTool.GET_LANGUAGES);
    }
    ...
    private void sendData() {
        try {
            // 1. URL
            URL url = new URL(WSTool.SERVER_URL + "/getlanguages");
            // 2. Open connection
            ...
            if (responseJSON.getInt("RES") == 0 && responseCode == 200) {
                mInformer.onSuccess(WSTool.GET_LANGUAGES, responseJSON);
            } else {
                mInformer.onError(WSTool.GET_LANGUAGES, responseJSON);
            }
